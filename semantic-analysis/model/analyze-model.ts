...
function getTableFields(ast_table: AST_Model.Table, list_ast_table: AST_Model.Table[], list_enum: Model.Enum[], list_table: Model.Table[]) {
  ...
  for (const field of ast_table.items) {
    if (field.name.text.includes('-')) {
      throw new Error(`Error line ${field.name.line} col ${field.name.col + field.name.text.indexOf('-')}: table field '${field.name.text}' contains illegal character '-'`);
    }

    const duplicate_column = findDuplicateColumn(field.name.text, ast_table.items);
    if (duplicate_column) {
      throw new Error(`Error line ${field.name.line} col ${field.name.col}: table field '${field.name.text}' existing name duplicate declaration on line ${duplicate_column.name.line} col ${duplicate_column.name.col}`);
    }
        ...
        switch (field.type.type.text) {
          ...
          default:
            if (isEnumExist(field.type.type.text, list_enum)) {
              ...
              throw new Error(`Error line ${field.type.type.line} col ${field.type.type.col} data type no-param '${field.type.type.text}' is not supported`);
            }
        }
        break;
      case "single-param":
        switch (field.type.type.text) {
          case 'varchar':
            ...
            break;
          default:
            throw new Error(`Error line ${field.type.type.line} col ${field.type.type.col} data type single-param '${field.type.type.text}(${field.type.p1.value})' is not supported`);
        }
        break;
      case "double-param":
        switch (field.type.type.text) {
          case 'decimal':
            ...
            break;
          default:
            throw new Error(`Error line ${field.type.type.line} col ${field.type.type.col} data type single-param '${field.type.type.text}(${field.type.p1.value}, ${field.type.p2.value})' is not supported`);
        }
        break;
      case "relation":
          ...
          switch (attr.value.type) {
            case "string":
              if (![
                ...
                throw new Error(`Error line ${field.name.line} col ${field.name.col} field '${field.name.text}' with string '${type.type}' is not compatible with number default value '${String(attr.value.name.text)}'`);
              }
              attributes.push({
                type: 'default',
                value: {
            ...
            case "number":
              if (![
                ...
                throw new Error(`Error line ${field.name.line} col ${field.name.col} field '${field.name.text}' with type '${type.type}' is not compatible with number default value '${String(attr.value.name.text)}'`);
              }
              attributes.push({
                type: 'default',
                value: {
            ...
            case "boolean":
              if (type.type !== 'boolean') {
                throw new Error(`Error line ${field.name.line} col ${field.name.col} field '${field.name.text}' with type '${type.type}' is not compatible with boolean default value '${String(attr.value.value)}'`);
              }
              attributes.push({
                type: 'default',
                value: {
            ...
            case "enum":
              if (type.kind !== 'enum') {
                throw new Error(`Error line ${field.name.line} col ${field.name.col} field '${field.name.text}' with type '${type.type}' is not compatible with enum default value '${String(attr.value.name.text)}'`);
              }
              const [enum_name, enum_value] = attr.value.name.text.split('.');
              if (!isEnumExistWithValue(enum_name, enum_value, list_enum)) {
                throw new Error(`Error line ${attr.value.name.line} col ${attr.value.name.col} enum value '${attr.value.name.text}' doesnt exist`);
              }
              attributes.push({
                type: 'default',
                value: {
...
export function analyzeModel(list_ast_enum: AST_Model.Enum[], list_ast_table: AST_Model.Table[], list_existing_enum: Model.Enum[], list_existing_table: Model.Table[]): AnalyzeModelResult {
  ...
  for (const ast_enum of list_ast_enum) {
    // TODO: check enum name duplicate
    if (ast_enum.name.text.includes('-')) {
      throw new Error(`Error line ${ast_enum.name.line} col ${ast_enum.name.col + ast_enum.name.text.indexOf('-')}: enum name '${ast_enum.name.text}' contains illegal character '-'`);
    }

    const existing_enum_index: number = list_enum.findIndex((e: Model.Enum) => e.name === ast_enum.name.text);
    if (ast_enum.extends) {
      if (existing_enum_index === -1) {
        throw new Error(`Error line ${ast_enum.name.line} col ${ast_enum.name.col + ast_enum.name.text.indexOf('-')}: extended enum '${ast_enum.name.text}' doesnt find existing enum`);
      }
      ...
      // enum exists
      if (existing_enum_index > -1) {
        throw new Error(`Error line ${ast_enum.name.line} col ${ast_enum.name.col + ast_enum.name.text.indexOf('-')}: enum '${ast_enum.name.text}' already defined before`);
      }
      list_enum.push({
  ...
  for (const ast_table of list_ast_table) {
    ...
    if (ast_table.extends) {
      if (existing_table_index === -1) {
        throw new Error(`Error line ${ast_table.name.line} col ${ast_table.name.col + ast_table.name.text.indexOf('-')}: extended name '${ast_table.name.text}' doesnt find existing table`);
      }
      ...
      if (existing_table_index > -1) {
        throw new Error(`Error line ${ast_table.name.line} col ${ast_table.name.col + ast_table.name.text.indexOf('-')}: table '${ast_table.name.text}' already defined before`);
      }
